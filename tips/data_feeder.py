    ...
    def decorate_reader(self,
        ...
        def __reader_creator__():
            if not multi_devices:
                role = os.getenv("PADDLE_TRAINING_ROLE",None)
                if role:
                    trainer_id = int(os.getenv("PADDLE_TRAINER_ID","0"))
                    trainers = int(os.getenv("PADDLE_TRAINERS","4"))
                    i = -1
                    for item in reader():
                        i=i+1
                        if i % trainers == trainer_id:
                            print("reader TRAINER_ID: {}, TRAINERS:{}, read: {}".format(trainer_id, trainers, i))
                ...
                if role:
                    ...
                    for batch in reader():
                        ...
                        if i % trainers == trainer_id:
                            ...
                            indexs.append(i)
                        if len(item) == num:
                            print("reader TRAINER_ID: {}, TRAINERS:{}, read: {}".format(trainer_id, trainers, ",".join(str(v) for v in indexs)))
